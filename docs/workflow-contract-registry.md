...
| Backend | `.github/workflows/master-pipeline-be.yml` | `templates/be-pipeline-caller.yml` | None from the caller template; defaults are provided for dispatch inputs. | `pipeline_mode`, `enable_security_scan`, `enable_sonar`, `enable_k6`, `k6_script_path`, `k6_base_url`, `k6_run_only_on_branch`, `run_deploy`, `run_promotion`, `dry_run`. | Secrets are inherited from the consumer repo; deployment, SonarCloud, k6, and promotion lanes use their matching repo secrets when enabled. | Caller grants `contents: write`, `pull-requests: write`, `packages: write`, `security-events: write`. | npm install lanes support GitHub Packages SDK auth with `GITHUB_TOKEN` / `NODE_AUTH_TOKEN`; consumer repos should not commit literal tokens. | Backend test, security, Docker, deployment, k6, and summary artifacts are uploaded by child workflows when enabled. |
